...
Basiselemente bilden die gestalterische und inhaltliche Grundlage einer Benutzeroberfläche.
Dazu gehören grundlegende visuelle und sprachliche Bestandteile wie Farben, Icons, Texte, Grafiken, Überschriften und Labels.

Sie sorgen dafür, dass Frontends konsistent, verständlich und wiedererkennbar gestaltet werden.
Gleichzeitig unterstützen sie die Benutzerfreundlichkeit, Orientierung und Barrierefreiheit innerhalb der Anwendung.
...
Die jeweils gültigen Farbdefinitionen sowie die zugehörigen Kontrastwerte sind vorrangig dem BVA-internen Styleguide zu entnehmen, übergreifend gilt der Styleguide der Bundesregierung:

* https://confluence.zssi.bva.in.bund.de/spaces/intranet014/pages/362416417/Corporate+Design#expand-Farben[BVA Corporate Design – Farben] _(nur im Behördennetzwerk erreichbar)_
* https://styleguide.bundesregierung.de/sg-de/basiselemente/farben[Styleguide der Bundesregierung – Farben]
...
Dabei sollten Icons primär zusammen mit <<text>> verwendet und nur in begründeten Fällen alleinstehend eingesetzt werden.
Wenn Icons alleinstehend als xref:06-bedienelemente.adoc#icon-button[Icon-Buttons] oder Aktionselemente genutzt werden, sollte ihre Anzahl auf das notwendige Minimum beschränkt werden.
Es wird empfohlen, alleinstehende Icons nur mit weit verbreiteter und im jeweiligen Kontext eindeutig verständlicher Bedeutung zu verwenden.

Alleinstehende interaktive Icons müssen über eine eindeutige zugängliche Bezeichnung verfügen.
Ist ihre Funktion anhand des Symbols und des Kontexts nicht unmittelbar verständlich, sollte zusätzlich ein Tooltip angezeigt werden.
Ein Tooltip ersetzt die zugängliche Bezeichnung nicht.
Tooltips müssen sowohl mit der Maus als auch über den Tastaturfokus zugänglich sein.
...
So werden xref:06-bedienelemente.adoc#button[Schaltflächen], <<label,Labels>> und andere interaktive Elemente gewöhnlich zuerst wahrgenommen, während statischer Text eher selten gelesen wird.
...
_Darstellung nicht-lateinischer Zeichen_

Anwendungen müssen die in der DIN 91379 vorgesehenen Unicode-Zeichen korrekt verarbeiten und darstellen.
Dazu gehören insbesondere nicht-lateinische Zeichen wie griechische oder kyrillische Schriftzeichen.

Als Standardschrift wird BundesSans verwendet.
Enthält die BundesSans ein benötigtes Zeichen nicht, wird für dieses Zeichen die Schriftart Liberation als Ersatzschrift verwendet.
In der technischen Umsetzung ist Liberation daher als nachgelagerte Schriftart in der Font-Fallback-Kette einzubinden.
Dabei ist zu beachten, dass bei Liberation das große I und das kleine l leicht verwechselt werden können.
...
Die Standard-Schriftart für xref:06-bedienelemente.adoc#bedienelemente[Bedienelemente] und Inhalte ist BundesSans (14 pt, regular).
...
Eine Ausnahme besteht darin, dass das erste Wort der Beschriftung eines xref:06-bedienelemente.adoc#bedienelemente[Bedienelements] immer großgeschrieben wird, unabhängig von der jeweiligen Wortart.
...
Stattdessen werden andere, platzsparende Mechanismen angewandt wie xref:07-design-pattern.adoc#expander[Expander (Progressive Disclosure)].
...
* Es muss sichergestellt sein, dass der xref:02-barrierefreiheit.adoc#barrierefreier-einsatz-von-farben[Kontrast der Schriftfarbe zur Hintergrundfarbe] ausreichend groß ist.
...
* Die Nutzung von Fachterminologie muss entsprechend konsistent erfolgen.
...
* Nutzung von unterstrichenem Text in Textpassagen (außer für xref:06-bedienelemente.adoc#link[Hyperlinks]).
...
Überschriften sollten grundsätzlich mit den dafür vorgesehenen Überschriftselementen umgesetzt werden.
ARIA-Rollen sind nur in begründeten Ausnahmefällen zu verwenden, wenn dies nicht möglich ist.
Sie stellen keinen gleichwertigen Ersatz für semantische Überschriften dar.
...
* *Schriftgewicht und Kursivschrift:*
Überschriften können abhängig von ihrer Hierarchie durch unterschiedliches Schriftgewicht hervorgehoben werden.
Kursivschrift wird in Überschriften nicht verwendet.
* *Farben:*
Überschriften werden ausschließlich dunkelgrau dargestellt.
Unterschiedliche Hierarchieebenen werden klar durch Schriftgröße, Schriftgewicht und Abstände voneinander abgegrenzt, nicht durch unterschiedliche Farben.

[[label]]
=== Label

Labels dienen dazu, xref:06-bedienelemente.adoc#eingabefeld[Eingabefelder] und andere xref:06-bedienelemente.adoc#bedienelemente[Bedienelemente] klar zu kennzeichnen, sodass Nutzende genau verstehen, welche Informationen erforderlich sind und welche Aktionen durchgeführt werden können.
Ein konsistentes und gut durchdachtes Design von Labels trägt wesentlich zur Verbesserung der Benutzererfahrung bei.

Labels sind nicht nur visuelle Hinweise, sondern auch wesentliche Bestandteile der xref:02-barrierefreiheit.adoc#barrierefreiheit[Barrierefreiheit].
...
Ein korrekt implementiertes Label sorgt dafür, dass alle Nutzenden, unabhängig von ihren Fähigkeiten, xref:05-benutzereingaben.adoc#benutzereingaben[Benutzereingaben] und andere interaktive Elemente problemlos nutzen können.
...
Labels können auch für interaktive Elemente wie xref:06-bedienelemente.adoc#button[Schaltflächen] und xref:06-bedienelemente.adoc#link[Links] verwendet werden.
Hier sollten xref:02-barrierefreiheit.adoc#aria-labels[ARIA-Labels] (aria-label) oder ARIA-Beschreibungen (aria-labelledby) eingesetzt werden, um die Zugänglichkeit zu verbessern.
Beispiel:
[source,xml]
...
Hier werden die Leitlinien für die Verwendung von Grafiken festgelegt, einschließlich Stilrichtlinien, Bildgrößen und im Ansatz deren xref:02-barrierefreiheit.adoc#barrierefreiheit[Barrierefreiheit].

Jedes Bild benötigt eine Bildunterschrift und ein aussagekräftiges xref:02-barrierefreiheit.adoc#alt-tag[Alt-Attribut], um die Barrierefreiheit von Frontends zu unterstützen.
...
* Grafiken müssen ein aussagekräftiges xref:02-barrierefreiheit.adoc#alt-tag[Alt-Attribut] besitzen.
