...
Im Folgenden werden die Richtlinien zur Barrierefreiheit gemäß Standards wie WCAG und andere Methoden zur Verbesserung der Zugänglichkeit erläutert.
...
* Die Textgröße soll standardmäßig mindestens 14pt betragen.
...
//ToDo: Welches Frontend? Ist hier der Buastein gemeint?
* Das Frontend ist primär für die Nutzung auf Desktopgeräten konzipiert, kann aber durch die grundlegende Responsivität auch auf mobilen Geräten genutzt werden.
...
Die Anwendungsfarbe ist frei wählbar, aber muss vom Projekt auf Barrierefreiheit geprüft werden.
...
In der folgenden Tabelle sind mögliche Icons, deren Funktionen sowie die dazugehörigen CSS-Klasse aufgeführt.
...
In Bezug auf Software und User-Interface-Design kann auf diese Tatsache jedoch nur eingeschränkt zurückgegriffen werden, denn Nutzende lesen die Texte auf einem Bildschirm nicht, vielmehr scannen sie diesen nach relevanten Informationen.

Verschiedenartige Informationen rufen dabei unterschiedliche Aufmerksamkeitsstufen hervor.
So werden <<button,Schaltflächen>>, <<label,Labels>> und andere interaktive Elemente gewöhnlich zuerst gescannt, während statischer Text eher selten gelesen wird.
...
Sie unterliegen in erster Linie der notwendigen Fachlichkeit.
...
* Schmuckgrafiken sind erlaubt, jedoch entsprechend zu kennzeichnen.
...
In Ausnahmefällen muss die Schriftgrafik bspw. durch Alt-Attribute ausreichend beschrieben sein.
...
Hyperlinks sind verlinkte <<text,Texte>> (oder auch <<grafiken>>), die im Wesentlichen als Navigations-Mechanismus dienen, zum Beispiel um neue Fenster mit anderen Inhalten zu öffnen. Wird die Maus über einen Hyperlink bewegt, ändert sich der Cursor vom Pfeil zum Handsymbol.
...
Altanwendungen, die nicht angepasst werden müssen, können weiterhin dem https://isyfact.github.io/isyfact-standards-doku/3.0.x/isy-bedienkonzept/bedienkonzept.html[zum Umsetzungszeitpunkt gültigen Bedienkonzept] entsprechen.
...
Aktive Eingabefelder werden mit einer 1px dickere Umrandung in der Highlightfrabe Blau dargestellt. Sie verlieren ihren Platzhalter und können entweder geleert werden (siehe Abbildung <<image-eingabefeldaktiv>>) oder eine Mask enthalten (siehe Abbildung <<image-eingabefeldmask>>).
...
Dies erleichtert den Nutzenden die Eingabe von festgelegten Formaten, beispielsweise die Eingabe eines Datums bei einem Date-Picker.
...
Bei der Eingabe von Text können Textboxen manuell in ihrer Größe verändert werden, sodass der gesamte Inhalt angezeigt werden kann. Textboxen sollten mindestens so groß gestaltetet werden, dass der am häufigsten zu erwartende Inhalt hineinpasst, ohne manuell die Box anzupassen.
...
Listen ermöglichen es dem Nutzenden, zulässige Werte auszuwählen.
...
* Checkboxen sind kein alleinstehender Ein- und Ausschalter, dafür gibt es die technische Komponente _<<input-switch,p-input Switch>>_.
...
Wenn keine oder eine Zeile auswählbar ist, dann erfolgen Folgeaktionen über die Aktionselemente.
...
Es kann vorkommen, dass es in einer Anwendung sinnvoll ist, große Datenmengen auszuwählen und dass es in einer anderen Anwendung wiederum besser ist, gar keine Auswahl zu zulassen. Grundsätzlich gilt, dass ausgewählte Zeilen farbig hinterlegt werden müssen. Alle nachfolgenden Punkte sind Möglichkeiten, die eingesetzt werden *können*.
...
Um für Nutzende den Umgang mit solchen Tabellen zu vereinfachen, sollten nicht alle Daten initial in der Tabelle angezeigt werden.
...
Sie können durch ihre inhaltlich logische Struktur dem grundlegenden dreispaltigen Layout folgen.
...
* Bei einer Veränderung der Browserfenster-Größe sollten sich die Breite der Spalten dynamisch an den verfügbaren Platz anpassen, ohne dabei die Lesbarkeit zu beeinträchtigen.
...
Feedback-Konzepte für Ladezeiten sind ein sehr wichtiges Instrument, um eine User Experience zu erreichen.
...
Beim Verfassen von System-Benachrichtigungen – wie bspw. <<fehlermeldungen>> – muss auf eine verständliche und konsistente Formulierung geachtet werden.
...
Um die <<barrierefreiheit>> einzuhalten, muss die Notification dauerhaft eingeblendet bleiben, bis sie von den Nutzende geschlossen wird, siehe https://www.w3.org/WAI/WCAG21/Understanding/content-on-hover-or-focus.html[WCAG 2.1 Kriterium 1.4.13a].
...
Fehlermeldungen sollten gut sichtbar, in verständlicher Sprache verfasst sein und klare Anweisungen zur Behebung des Fehlers enthalten.
...
Ein häufig auftretendes Problem ist dabei die Verwendung von negativ anmutenden Formulierungen, die Benutzende demotivieren können.
