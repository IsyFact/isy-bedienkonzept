...
Die Seiten-Toolbar kommt auf Detailseiten von Dashboards und Applikationen zum Einsatz.
...
Weitere Aktionen werden als Icon-Buttons mit eindeutigem sichtbaren Textlabel oder `aria-label` bereitgestellt.
Die Beschriftung ist konsistent und normgerecht auszuführen.

* *Responsives Verhalten:* 
  ** Die Toolbar ist einzeilig und ohne horizontales Scrollen darzustellen.
  ** Sekundäre Aktionen werden bei schmalen Viewports stufenweise reduziert
  (Labelkürzung → Gruppierung → Overflow-Menü).
  ...
  ** Definierte Breakpoints sind einzuhalten; Layoutsprünge sind auszuschließen.

* *Barrierefreiheit:* 
** Benennungen und Reihenfolgen gelten anwendungsweit verbindlich.
** Die Toolbar ist vollständig tastaturbedienbar.
** Fokusindikatoren sind sichtbar und Kontraste erfüllen mindestens WCAG AA.

[[tabellen-toolbar]]
*_Tabellen-Toolbar_*

Die Tabellen-Toolbar steht unmittelbar oberhalb einer Tabelle und bündelt Aktionen für die Tabelle sowie für ausgewählte Datensätze.
...
* *Darstellung & Erkennbarkeit:* Buttons sind vorrangig als Kombination aus Icon und Text bereitzustellen.
...
* *Filter rechts platzieren:* Ein Filter-Toggle zur Eingrenzung der angezeigten Daten ist immer rechtsbündig auszurichten.
Der Zustand (aktiv/inaktiv) ist eindeutig zu kennzeichnen.
Die Auslösung öffnet eine Filterleiste oder ein Panel.

* *Reihenfolge & Benennung:* Tabellenbezogene Aktionen stehen links; primäre Aktionen zuerst, destruktive Aktionen eindeutig benannt (z. B. „Löschen“).
Benennungen und Reihenfolgen sind anwendungsweit konsistent einzuhalten.

* *Responsives Verhalten:*
** Bei schmalen Viewports werden Buttons gruppenweise reduziert (zuerst Sekundäraktionen) und optional in ein Overflow-Menü überführt.
** Horizontales Scrollen ist auszuschließen.
** Eine sticky-Ausführung ist zulässig, sofern die Tabellenüberschrift nicht überdeckt wird.
** Labels dürfen per Ellipsis verkürzt werden. Bedeutungstragende Icons sind beizubehalten.

* *Barrierefreiheit:*
** Die Toolbar ist mit `role="toolbar"` und einem zugänglichen Label (z. B. `aria-label="Tabellen-Aktionen"`) auszuzeichnen.
** Icon-only-Aktionen erhalten einen eindeutig ermittelbaren Accessible Name.
** Der Filter-Toggle nutzt `aria-pressed`.
** Die Fokusreihenfolge entspricht der visuellen Ordnung und der Fokusindikator ist sichtbar.
** Auswahlabhängige Aktionen werden deaktiviert oder mit einem Zähler versehen (z. B. „Löschen (3)“).
** Änderungen der Auswahl sind über ein Status-Element (z. B. `role="status"`) auszugeben.

[[eingabefeldgruppen-toolbar]]
*_Eingabefeldgruppen mit Toolbar_*

Auf Detailseiten kann eine Toolbar zur Bearbeitung von Objekten innerhalb von Eingabefeldgruppen eingesetzt werden.
Die Ausgestaltung folgt den Vorgaben aus <<toolbar>>.
...
* *Zweck:* Sie stellt Funktionen zur Bearbeitung der Objekte bereit, die innerhalb der jeweiligen Eingabefeldgruppe enthalten sind.
...
* *Platzierung:* Die Buttons sind rechts neben der Gruppenüberschrift anzuordnen und innerhalb des Button-Clusters linksbündig auszurichten.

* *Responsives Verhalten:*
** Der Kopfbereich der Gruppe ist nach Möglichkeit einzeilig auszuführen.
** Horizontales Scrollen ist auszuschließen.
** Sekundäre Aktionen werden bei schmalen Viewports gebündelt oder in ein Overflow-Menü überführt.
** Die Reihenfolge (Primär → Sekundär → Destruktiv → Mehr) bleibt verbindlich bestehen.
** Falls ein Umbruch erforderlich ist, ist die Toolbar auf einer zweiten Zeile unterhalb der Überschrift darzustellen – ohne Änderungen der Aktionsreihenfolge und ohne Layoutsprünge.

* *Barrierefreiheit:*
** Die Gruppenüberschrift ist als semantische Überschrift auszuführen.
** Die Toolbar erhält `role="toolbar"` und ein eindeutiges Label (z. B. `aria-label="Aktionen: Adressdaten"`).
** Icon-Buttons verfügen über einen eindeutig ermittelbaren Accessible Name.
** Toggle-Aktionen nutzen `aria-pressed`.
** Die Fokusreihenfolge entspricht der visuellen Ordnung und der Fokusindikator ist sichtbar.
** Beim Öffnen eines Overflow-Menüs wandert der Fokus in das Menü, beim Schließen zurück zum Auslöser.
** Nicht verfügbare Aktionen werden deaktiviert statt ausgeblendet.
** Kontraste erfüllen mindestens WCAG AA.
