...
Das Bedienkonzept nimmt eine zentrale Rolle in der Entwicklung von xref:isyfact-standards-doku:isyfact-standards-doc:referenzarchitektur:software-technisch/frontend.adoc[Frontends] ein.
...
Darauf folgen die *<<basiselemente>>* (wie z.B. <<text>> und <<farben>>), die den Grundstein für die nachfolgenden Kapitel legen und von essenzieller Bedeutung sind.
Danach kommen die *<<bedienelemente>>*, die einfachste Ausprägung eines Interaktionselements.
*<<design-pattern>>* stellen eine Menge aus Bedienelementen und Basiselementen dar und können letztendlich in beliebiger Kombination als vollständige *<<use-cases>>* beschrieben werden.
...
Die Gültigkeit des https://isyfact.github.io/isyfact-standards-doku/3.0.x/isy-bedienkonzept/bedienkonzept.html[Bedienkonzepts für Altanwendungen] bleibt zum damaligen Zeitpunkt mit den damalig gültigen Inhalten weiterhin aktiv.
...
Im Design bezieht sich das Prinzip der "Klarheit" darauf, wie verständlich ein Frontend und wie einfach es zu navigieren ist.
...
Dies kann in Form von visuellem Feedback, wie zum Beispiel einer Farbänderung eines <<button,Buttons>> nach einem Klick, oder durch informative Nachrichten, wie Bestätigungsmeldungen nach dem Absenden eines <<formulare,Formulars>>, erfolgen.
...
Für Haupt- & Einstiegsseiten, sowie Seiten mit rechtlicher Relevanz muss der Kontrast 7:1 betragen.
...
* <<grafiken,Bilder>>, die ausschließlich der Gestaltung dienen (z.B. Trennlinien), müssen als solche (Schmuckgrafik) gekennzeichnet werden.
* *Tastatur-Navigation:*
...
Wenn nötig, geben Sie hierfür spezifische Sprachvarianten oder Dialekte mithilfe von Codes für Regionen oder Länder an (z.B. en-US für amerikanisches Englisch, en-GB für britisches Englisch).
...
* Die Verwendung einer Anwendungsfarbe muss vom Projekt auf Barrierefreiheit geprüft werden und ist frei wählbar.
...
Icons können als zusätzliches Element genutzt werden, um Funktionen und Inhalte auch visuell kenntlich zu machen.
...
In Bezug auf Software und User Interface Design kann auf diese Tatsache jedoch nur eingeschränkt zurückgegriffen werden, denn Nutzende lesen die Texte auf einem Screen nicht, vielmehr wird dieser nach relevanten Informationen gescannt.
...
zum Einsatz.
Diese Schriftart wird nur fehlende UNICODE-Zeichen in der BundesSans ergänzen, wodurch eine Unkonformität mit der BITV durch eventuelle Verwechslungen mit einem großen I und kleinem l ausgeschlossen werden kann.
...
Die fehlerhafte Anwendung von Texten kann somit direkt zu einer Reduzierung der Usability des entsprechenden Frontends führen.

Die Standard-Schriftart für <<bedienelemente>> und Inhalte ist BundesSans 14pt regular in dunkelgrau (#212529).
...
Individuelle Schriftgrößen können Sie mit dem Schriftgrößenrechner auf https://www.leserlich.info/werkzeuge/schriftgroessenrechner/index.php[leserlich.info] ermitteln und entsprechend an den Nutzungskreis anpassen.
...
Während für englischsprachige Frontends zwischen verschiedenen Formen der Groß- und Kleinschreibung unterschieden wird, muss für deutschsprachige Frontends lediglich die korrekte deutsche Rechtschreibung angewendet werden.
...
Hier ist der Kontrast größtmöglich zu wählen (z.B. dunkelgraue Schrift auf weißem Hintergrund).
...
* <<ueberschriften>> und großer/dicker Text muss mindestens einen Kontrast von 3:1 aufweisen.
...
Des Weiteren soll die Bildgröße dem Inhalt entsprechend gewählt werden, damit Nutzenden die Informationen übermittelt werden und kein horizontales Scrolling entsteht.
...
Gründe dafür sind, dass ihre Farben und Schriftgrößen nicht individuell eingestellt werden können und bei der Zoomvergrößerung die Schriftkanten meist unscharf werden.
...
* Bilder dürfen keine Schriftgrafiken enthalten, mit Ausnahme von Logos; in Ausnahmen muss die Schriftgrafik durch z.B. Alt-Attribute ausreichend beschrieben sein.

[[ueberschriften]]
=== Überschriften

Überschriften ermöglichen es Nutzenden, schnell die Hauptthemen einer Seite zu erfassen und sich zu orientieren.
Es werden Überschriften, Hierarchien und das visuelle Erscheinungsbild definiert, um Konsistenz und eine optimale Benutzererfahrung sicherzustellen.
...
* *Farben:*
<<farben>> können verwendet werden, um die Hierarchie zu betonen, sollten jedoch mit Bedacht gewählt werden, um ein harmonisches Gesamtbild zu bewahren.
...
Primäre, einleitende und abschließende Aktionen können sehr präsent in der Mitte des Frontends angeordnet werden.
...
* Zum Ein- oder Ausschalten von Inhalten werden spezielle <<input-switch,Input Switches>> verwendet.
...
Hyperlinks sind verlinkte <<text,Texte>> (oder auch <<grafiken>>), die im Wesentlichen als Navigations-Mechanismus dienen – so z.B. um andere Inhalte wie Fenster zu öffnen.
...
Altanwendungen, die nicht angepasst werden müssen, können weiterhin ihrem https://isyfact.github.io/isyfact-standards-doku/3.0.x/isy-bedienkonzept/bedienkonzept.html[zum Umsetzungszeitpunkt gültigen Bedienkonzept] entsprechen.
Alle hier gezeigten Grafiken beziehen sich auf die aktuelle und zukünftige Umsetzung.

Eingabefelder sind dynamisch.
Sie bestehen aus dem Eingabefeld selbst, einem vorbelegten <<placeholder,Placeholder>> und optional aus einer daraus entstehenden <<eingabefeld-mit-mask,Vorschau (Mask)>> zur Anzeige des erwarteten Formats.
...
Im normalen Zustand ist das Eingabefeld mit einem bündigen, statischen <<label>> in Dunkelgrau versehen.
...
Wenn eine Eingabe in einem Eingabefeld getätigt wurde und das Feld wieder verlassen wird, wechselt das Eingabefeld seinen Zustand wieder auf die Werte des „normalen“ Eingabefelds.
...
Das Eingabefeld färbt sich rot und es wird ein inhaltlich passender Hinweistext angezeigt, damit die Eingabe korrigiert werden kann.
...
Soll z.B. eine Postleitzahl innerhalb Deutschlands eingegeben werden, so macht es Sinn die Feldlänge auf 5 Stellen zu begrenzen.
...
Dies erleichtert den Nutzenden die Eingabe von festgelegten Formaten, bspw. bei einem Date-Picker.
...
* Erwäge die Nutzung einer Radiobutton-Gruppe bei einer kleinen Liste.
...
* Das Ein- oder Ausschalten einer Checkbox kann dazu genutzt werden, um andere Elemente dynamisch zu aktivieren/anzuzeigen oder zu deaktivieren/unsichtbar zu machen, z.B. „Abweichende Lieferanschrift“ (einzig zulässige Funktion).
...
* Bleibe konsistent und nutze entweder <<checkbox,Checkboxen>> oder Input Switches.
...
Neben <<image-eingabefelderror,Fehlermeldungen, die am Eingabeobjekt auftreten können>>, kann zusätzlich ein zusammenfassender Hinweis angezeigt werden (*Achtung:* dynamisch nachgeladene Elemente müssen auch einem Screenreader mitgeteilt werden, sodass auch Personen mit Einschränkungen diese Hinweise mitbekommen).
...
Neben Fehlermeldungen, die am Eingabeobjekt auftreten können, kann zusätzlich ein zusammenfassender Hinweis, wie im Kapitel <<system-meldungen>> beschrieben, angezeigt werden.
