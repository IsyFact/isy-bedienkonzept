...
Für Altanwendungen gilt, dass das xref:3.1.x@isyfact-standards-doku:isy-bedienkonzept:bedienkonzept.adoc[Bedienkonzept (beziehungsweise ehemals Styleguide) zum Zeitpunkt der Entwicklung der Altanwendung] mit den damals geltenden Inhalten gültig bleibt.
...
Altanwendungen, die nicht angepasst werden müssen, können weiterhin dem xref:3.1.x@isyfact-standards-doku:isy-bedienkonzept:bedienkonzept.adoc[zum Umsetzungszeitpunkt gültigen Bedienkonzept] entsprechen.
