...
Bei der Gestaltung des Labels gibt es zwei Möglichkeiten. Das Label ist "regular" oder "bold".
...
.Eingabefeld aktiv und "bold"-Label
...
.Eingabefeld befüllt, nicht aktiv und "bold"-Label
image::24_BE_eingabefeld_filled.png[align="center"]
...
.Eingabefeld Read Only
image::24_BE_eingabefeld_read_only.png[align="center"]

oder

[[image-eingabefeldreadonly-bold]]
.Eingabefeld Read Only mit "bold"-Label
image::24_BE_eingabefeld_bold_readonly.png[align="center"]

Im Gegensatz zu einem deaktivierten Eingabefeld sind Read Only Eingabefelder zugänglich und relevant. Deaktivierte Felder haben einen Standardhintergrund und sehr hellen Rand.
Die Information lässt sich aus dem Feld auslesen (auch durch Screenreader oder andere Unterstützungstechnologien) zum Beispiel durch "StrG+C".
...
Die "Pflicht"-Kennzeichnung erfolgt dabei durch das Label und wird durch ein Asterisk ("*") dargestellt.
Das Asterisk wird immer ans Ende eines Labels gestellt.
Pflichtfelder haben immer eine Validierung und eine entsprechende Fehlermeldung.

[NOTE]
Ein Pflichtfeld kann zusätzlich durch die Deklaration des Platzhalters = "Pflichtfeld" oder "required" gekennzeichnet werden.
...
.Eingabefeld mit Fehlermeldung mit "bold"-Label
...
Ein Eingabefeld hat keine fixe Länger.
Ein Eingabefeld ist so lang, wie es der fachlich Kontext notwendig macht. Wenn 95% der erwarteten Eingaben deutlich kürzer sind als Einzelfälle ist die Länge so zu wählen, dass 95% der Eingaben ohne Interaktion vollständig zu lesen sind.
Die Mindesthöhe eines Eingabefelds von 44px darf nicht unterschritten werden, um die Barrierefreiheit von Input-Optionen zu gewährleisten.
...
.Eingabefeld mit Mask mit "bold"-Label
...
.Exemplarisch Eingabefeld mit Input-Option Sonderzeichen
image::24_BE_eingabefeld_input.png[align="center"]

oder

[[image-eingabefeldinputoption]]
.Exemplarisch Eingabefeld mit Input-Option Sonderzeichen mit "bold"-Label
...
Der Sonderzeichen-Picker ist eine Inputoption um nicht übliche Zeichen ohne weitere Hilfsmittel eingeben zu können.
Die Inputoption ist immer an Ende eines Eingabefelds und orientiert sich am Styling eines "outlined-Button". Border und Icon sind in der Primärfarbe gestaltet.
...
Bei Klick auf die Inputoption öffnet sich mittig als Overlay der Sonderzeichenpicker.
...
[[mehrfacheingabe]]
=== Mehrfacheingabe eines Eingabeobjekts
...
Read-Only Zustände und Disabled habe klar definierte Unterschiede die gezielt genutzt werden können.

*_"Read Only" State_*
Ein "Read Only"-Feld erlaubt es, die darin enthaltenen Daten zu sehen, jedoch nicht zu ändern. Dies ist besonders dann nützlich, wenn Informationen relevant sind, jedoch nicht modifiziert werden sollen.

*Merkmale:*

* Sichtbarkeit: Der Nutzende kann den Inhalt des Feldes vollständig sehen und kopieren.
...
* Bestätigungsseiten: Nach der Eingabe eines Formulars kann ein "Read Only"-Zustand verwendet werden, um die eingegebenen Daten zur Überprüfung anzuzeigen.
* Informationsanzeige: Felder, die wichtige Daten enthalten, die jedoch durch den aktuellen Benutzer nicht verändert werden sollen (z.B. automatisch generierte ID-Nummern, erfasste Zeitstempel).

*Handlungsempfehlungen:*

Verwende "Read Only" für Felder, deren Inhalte für den Benutzer relevant sind, aber nicht verändert werden dürfen.
Gestalte das Design so, dass die visuelle Unterscheidung klar, aber nicht störend ist – z.B. durch subtile Farbveränderungen.
Erlaube den Benutzern weiterhin, Inhalte aus dem "Read Only"-Feld zu kopieren, falls sie die Daten anderweitig benötigen.

*_"Disabled" State_*
Ein "Disabled"-Feld hingegen ist vollständig inaktiv. Der Benutzende kann den Inhalt nicht oder schlecht sehen, nicht bearbeiten und auch keine Interaktion (wie Kopieren) durchführen.
Diese Option wird verwendet, um dem Benutzer zu signalisieren, dass bestimmte Funktionen oder Eingaben derzeit nicht zur Verfügung stehen.
...
* Zugriffssteuerung: Felder, die nur von Benutzenden mit bestimmten Rechten bearbeitet werden dürfen, können für andere Nutzende "disabled" sein.
...
Nutze "Disabled" für Felder, die dem Benutzer derzeit nicht zugänglich sein sollen, um Fehlinteraktionen zu verhindern.
Vermeide "Disabled" für wichtige Informationen, die der Benutzer sehen muss – in solchen Fällen ist "Read Only" besser geeignet.
Mache den "Disabled"-Status klar ersichtlich, indem du eine deutliche visuelle Abgrenzung durch Ausgrauen oder andere farbliche Mittel verwendest.

*Vergleich*

Der wesentliche Unterschied zwischen "Read Only" und "Disabled" liegt in der Sichtbarkeit und der Interaktivität der Felder.
Während "Read Only"-Felder dem Benutzenden Informationen präsentieren, die er nicht bearbeiten darf, aber weiterhin sehen und kopieren kann, sind "Disabled"-Felder vollständig gesperrt und weder sichtbar noch interaktiv.

Für die Anwendung in Formularen sollten folgende Grundsätze beachtet werden:

"Read Only" ist ideal, wenn die Informationen wichtig sind und für den Benutzenden sichtbar bleiben müssen, aber nicht verändert werden sollen.
"Disabled" eignet sich, wenn der Benutzende keine Informationen im Feld sehen oder bearbeiten soll – z.B. bei unzugänglichen Funktionen oder Bedingungen, die erst später erfüllt werden müssen.
