...
Die technische Umsetzung kann mit Angular sowie PrimeNG/PrimeFlex erfolgen.
...
Sie werden als Kacheln oder Panels mit Titel, optionalem Icon und kurzer Beschreibung umgesetzt (z. B. `p-card`/`p-panel`).
...
| Die Entscheidung erfolgt im Kontext; minimaler Fokuswechsel; optionale Rückgängig-Option per Toast
| `p-popover`, `p-toast`

| Kontextnahe Bearbeitung (ein Formularabschnitt)
| Drawer/Side Panel (nicht blockierend)
| Der Arbeitskontext bleibt sichtbar; geringere kognitive Last als Modal
| `p-sidebar`

| Mehrschritt-Anlage oder komplexer Flow
| Seiteneigener Stepper (nicht modal)
| Bessere Orientierung, History und Deeplinks sind möglich; Inline-Validierung
| `p-stepper`, `p-message`

| Rechtlich/irreversibel/hochriskant
| Modaler Dialog (nur wenn zwingend)
| Explizite Willensbekundung ist erforderlich; Input-Fokus ist abgesichert
| `p-dialog [modal]="true"`

| Systemhinweis oder Status
| Toast/Snackbar (global) oder Inline-Message
| Nicht blockierend; Screenreader-kompatibles Live-Region-Verhalten
| `p-toast`, `p-message`

| Hilfe oder Kontextinfo
| Popover oder Expander (inline)
| Zusatzinfos ohne Unterbrechung des Workflows
| `p-popover`, `p-accordion`, `p-panel`
|===
...
| Modeless
| Blockiert den Hintergrund nicht, überlagert nur den Inhalt. Der Einsatz ist für ergänzende, kurze Aufgaben vorgesehen (z. B. Details bearbeiten).
| `p-dialog [modal]="false"`

| Sheet (Bottom/Side Sheet)
| Am Rand angedockt, weniger intrusiv. Der Einsatz ist für mobile bzw. kompakte Bestätigungen oder sekundäre Panels vorgesehen.
...
Die Muster berücksichtigen die Anforderungen an Tastatur- und Screenreader-Bedienbarkeit gemäß Kapitel <<barrierefreiheit>> und greifen auf Angular und PrimeNG/PrimeFlex zurück (Komponenten dienen als Referenz).
...
* *Angedockte Bestätigung am Auslöser:* Neben dem auslösenden Element erscheint ein kompakter Popover oder ConfirmPopup.
...
* *Inline-Leiste:* Für Inline-Bestätigungen kommt eine Message-Komponente oder ein eigener Inline-Container mit `p-button` zum Einsatz.

* *Angedockt:* Für angedockte Bestätigungen ist `p-popover` oder `p-confirmPopup` mit klar beschrifteten Aktionen vorgesehen.

* *Rückgängig-Option:* Für die Rückgängig-Option kommt `p-toast` (via `MessageService`) mit interaktiver Aktion (Button oder Link) zum Einsatz.
...
* *Rückmeldung:* Erfolgsmeldungen erscheinen als nicht blockierende Hinweise (z. B. inline oder als Toast).
...
* *Formular-Implementierung:* Für die Umsetzung kommt Angular `FormGroup` mit PrimeNG-Inputs (`p-inputText`, `p-dropdown` usw.) und konsistenter, feldnaher Fehleranzeige zum Einsatz.

* *Tabellen-Edit:* Die Bearbeitung in `p-table` erfolgt als Zeilen- oder Zellen-Edit mit Speichern und Abbrechen.
Bei hoher Komplexität ist zusätzlich pro Zeile ein Drill-Down-Einstieg vorzusehen.

* *Fehlermeldungen:* Die feldnahe Rückmeldung erfolgt über `p-message`, optional zusätzlich global über `p-toast` (z. B. für zusammenfassende Hinweise).
...
* *Routing:* Der Angular Router unterstützt eine Rückkehrlogik für Fokus und Scrollposition.

* *Orientierung:* Optional ist `p-breadcrumb` einsetzbar; im Kopfbereich steht eine `p-toolbar` mit Zurück-Button (`p-button`).

* *Struktur:* Für die Strukturierung kommen `p-card` oder `p-panel` sowie `p-tabView` oder `p-accordion` zum Einsatz.

* *Validierung und Feedback:* Die feldnahe Rückmeldung erfolgt über `p-message`, optional zusätzlich global über `p-toast`.

* *Ungespeicherte Änderungen:* Das Abfangen erfolgt über einen Angular Guard; die Bestätigung erfolgt bevorzugt nicht-modal (Popover oder Inline-Leiste), ein Meldungsdialog ist nur bei drohendem Datenverlust zulässig (vgl. <<meldungsdialoge>>).
...
* *Globale Benachrichtigungen:* Für asynchrone Ereignisse (Erfolg, Hinweis, Warnung, Fehler) kommen Toast- oder Snackbar-Meldungen zum Einsatz.
...
* *Schließen und Timeout:* Info-Toasts dürfen automatisch ausblenden.
...
* *Global:* Für Ereignisse außerhalb des direkten Feldkontexts kommt `p-toast` (mit `MessageService`) zum Einsatz.

* *Inline:* Für feld- oder abschnittsnahe Hinweise ist `p-message` vorgesehen.
...
Als Technologie-Referenz dient der PrimeNG Stepper (ab Version 20) oder eine gleichwertige Umsetzung; das Layout erfolgt mit PrimeFlex oder CSS-Grid.
...
Fehler dürfen nicht ausschließlich als Toast ohne feldnahe Erklärung erscheinen.
...
* *Stepper-Komponente:* Für die Umsetzung kommt `p-stepper` (ab Version 20) zum Einsatz.
Die Schrittanzeige ist als `p-steplist` mit `p-step`-Elementen strukturiert.

* *Inhaltsbereich:* Die Inhalte der einzelnen Schritte sind in `p-steppanels` mit `p-steppanel`-Elementen organisiert.

* *Steuerleiste:* Für die Navigation kommen `p-button`-Elemente zum Einsatz; die Platzierung erfolgt konsistent am unteren Rand des Inhaltsbereichs.

* *Validierung und Feedback:* Die feldnahe Rückmeldung erfolgt über `p-message`, optional zusätzlich global über `p-toast` (z. B. für zusammenfassende Hinweise).
...
Um die <<barrierefreiheit>> einzuhalten, muss die Notification dauerhaft eingeblendet bleiben, bis sie von den Nutzenden geschlossen wird, siehe https://www.w3.org/WAI/WCAG21/Understanding/content-on-hover-or-focus.html[WCAG 2.1 SC 1.4.13].
...
:example-link-component: Toast-Notification
