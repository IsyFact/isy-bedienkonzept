...
Sie folgen den im Kapitel <<benutzereingaben>> definierten Grundprinzipien.
...
[NOTE]
Für alle Design Patterns gelten die allgemeinen Anforderungen an Barrierefreiheit, Fokusführung und responsives Layoutverhalten gemäß den Kapiteln <<barrierefreiheit>> und <<fenstertypen-layout>>.
...
[[r-dashboard-grid]]
* *Struktur:* Die Teilbereiche sind klar voneinander abgegrenzt und folgen einem responsiven Raster (vgl. <<layout-resizing>>).
...
** Dashboard-Widget - <<image-farbmarkierung,Farbmarkierung>> und Applikationsicon
...
Kontextnahe, nicht blockierende Muster (z. B. Drawer/Sidebars, Popover oder Inline-Leisten) sind grundsätzlich gegenüber stark modalen Lösungen zu bevorzugen.
Modale Dialoge kommen ausschließlich dann zum Einsatz, wenn sie fachlich zwingend erforderlich sind (vgl. <<modalarme-interaktionen>>).
...
[[r-overlays-reflow]]
* *Reflow und Responsivität:* Overlays sind im Rahmen der Reflow-Anforderungen ohne horizontales Scrollen nutzbar.
Layoutsprünge bei Zustandsänderungen sind zu vermeiden.

[cols="3,3,4", options="header"]
|===
| Interaktionsfall | Empfohlener Container | Begründung
...
Die Anordnung passt sich responsiv an und bleibt auch in schmalen Viewports übersichtlich (vgl. <<layout-resizing>>).
...
| Sheet
| Ist am Rand des Viewports angedockt und weniger intrusiv als ein modaler Dialog. Der Einsatz ist für sekundäre Aufgaben, Detailinformationen oder ergänzende Bearbeitungen vorgesehen.
...
* Nicht fortsetzbare Fehlerzustände erfordern eine unmittelbare Entscheidung (z. B. Sitzung abgelaufen, Authentifizierung fehlgeschlagen).
...
Die Muster berücksichtigen die Anforderungen an Tastatur- und Screenreader-Bedienbarkeit gemäß Kapitel <<barrierefreiheit>>.
...
Die Ankündigung von Statusmeldungen erfolgt über eine Live-Region, ohne dass sich der Fokus verschiebt.
...
* *Mobil:* Touch-Ziele erfüllen die Mindestgröße und den Mindestabstand gemäß den geltenden <<barrierefreies-layoutverhalten,Barrierefreiheitsvorgaben>>.
...
* *Text statt Farbe:* Schweregrade sind sprachlich zu benennen (z. B. „Warnung“, „Fehler“).
...
Das Layout folgt dem allgemeinen Raster gemäß <<layout-resizing>>.
...
* Markierung und <<system-meldungen,Rückmeldung>>: Markierte Elemente sollten visuell hervorgehoben und durch dynamische Hinweise bestätigt werden.
