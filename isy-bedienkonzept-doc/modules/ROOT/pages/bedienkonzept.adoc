...
Zugängliches Design beinhaltet die Verwendung von klaren und verständlichen <<text,Texten>>, <<barrierefreier-einsatz-von-farben,ausreichendem Kontrast>> zwischen Text und Hintergrund, Tastaturzugänglichkeit, <<alt-tag,alternative Texte für Bilder>> und die Vermeidung von Technologien (z.B. Gestensteuerung), die für einige Personengruppen hinderlich sein könnten.
...
Für Haupt- & Einstiegsseiten und Seiten mit rechtlicher Relevanz muss der Kontrast 7:1 betragen.
...
Hier ist der Kontrast größtmöglich zu wählen (z.B. dunkelgraue Schrift auf weißen Hintergrund).
...
* Es ist darauf zu achten, dass das Bildmaterial ausreichend hoch aufgelöst ist, sodass keine einzelnen Fragmente zu erkennen sind.
...
Im "aktiven" bzw. "fokussierten" Zustand bekommt der Button einen Fokus-Rahmen, der einen helleren Ton hat als die Button-Rahmenfarbe.
...
Eine Mischung aller farblichen kolorierten und invertierten Buttons ist jedoch nicht zulässig.
...
Sie bestehen aus dem Eingabefeld an sich, einem vorbelegten <<placeholder,Placeholder>> und optional aus einer daraus entstehenden <<eingabefeld-mit-mask,Vorschau (Mask)>> zur Anzeige des erwarteten Formats.
...
Sie haben zusätzlich zur veränderten Farbe eine 1px dickere Border.
...
Das Eingabefeld färbt sich rot und es wird ein inhaltlich logischer Hinweistext angegeben, damit die Eingabe korrigiert werden kann.
...
Die Schriftgröße der Labels, Platzhalter und Hinweistexten sollte 14pt nicht unterschreiten.
...
* Es können gleichzeitig mehrere Filter auf unterschiedlichen Spalten gesetzt werden.
...
So kann es sein, dass es in einer Anwendung Sinn macht eine große Datenmenge auszuwählen und in einer anderen Anwendung wiederum gar keine Auswahl zuzulassen.
...
//ToDo: Hier weiter mit Verweisen, Review, etc.
[[menu-button]]
=== Menü-Button
