...
So gibt es noch weitere Dos and Dont's im Rahmen der Tastatursteuerung zu beachten:
...
* *Feedback:* <<fehlermeldungen, Fehlermeldungen>> und Erfolgshinweise werden sowohl visuell als auch über Screenreader wahrnehmbar angezeigt.
* S**tandardbedienmuster:** Es werden nur Standard-Tastenkombinationen verwendet, um Konflikte mit Browserfunktionen oder Assistenzsoftware zu vermeiden.
...
Weitere Kombinationen im Browser zur Steigerung der Effektivität sind (kann in seltenen Fällen abweichen):
...
** <<tastatursteuerung, Tastaturzugänglichkeit>>: Wertehilfen müssen vollständig mit der Tastatur bedienbar sein, einschließlich <<navigation, Navigation>>, Auswahl und Schließen.
...
Die für die IsyFact gültigen Wertehilfen basieren auf PrimeNG-Komponenten, diese werden ständig weiterentwickelt und aktualisiert und kann optisch von dem hier dokumentierten Stand abweichen.
...
Genaue Details zur Umsetzung eines Dropdowns sind im Kapitel Bedienelemente, <<dropdown, Dropdown>> zu finden.
...
*_Funktionalität:_* Ermöglicht das Hinzufügen oder entfernen einer oder mehrerer kontextbezogenen gruppierten Eingabefeldern und Eingabemöglichkeiten (zum Beispiel "Wohnort", bestehend aus mehreren Adressinformationen).

*_Bedienung:_*

* Maus: Klick auf „+ weiteres Element“ erzeugt eine weitere Context Group, klick auf Delete-Icon löscht die ausgewählte Context-Group.
* Tastatur: Tabulator zur Fokussierung, Pfeiltasten zur Navigation, Enter zur Auswahl.
...
Die genaue Definition und Umsetzung der einzelnen Elemente ist im entsprechenden Unterkapitel des Kapitels <<bedienelemente, Bedienelemente>> einzusehen.
...
*_Funktionalität:_* Filter ermöglichen das Eingrenzen von Datenmengen anhand vordefinierter Kriterien (zum Beispiel Kategorien, Preisbereiche).
...
* Sicherstellung, dass Screenreader alle Interaktionen klar kommunizieren (zum Beiospiel „X Ergebnisse gefunden“).
...
*_Funktionalität:_* Durch einen Klick auf "Bearbeiten" wird eine Zeile in den Bearbeitungsmodus versetzt, und Eingabefelder werden aktiviert.
...
Löschen ermöglicht das direkte Löschen einer gesamten Tabellenzeile.
Dies verbessert die Effizienz, da Benutzende nicht zwischen Ansichts- und Bearbeitungsmodi wechseln müssen.
Zur Vermeidung von versehentlichen Aktionen sollte die Löschfunktion zusätzlich über einen Modaldialog (zum Beispiel: Sind Sie sich sicher, diesen Datensatz zu löschen? Ja/Nein) abgesichert werden.
