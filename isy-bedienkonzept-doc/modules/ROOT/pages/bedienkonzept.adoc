...
* *Passwortmanager:* Das Einfügen (Paste) und Autofill sind zuzulassen.
...
Leer-, Lade- und Fehlerzustände sind stabil auszuprägen (Skeleton/Spinner, klare Meldung, Retry).
...
* *Modalarme Interaktionen:* Bestätigungen sind bevorzugt inline bereitzustellen, ergänzt durch Rückgängig-Optionen (Toast).
...
Sie stellt zentrale Inhalte und Funktionen bereit und führt zu Unterseiten.
...
* *Linksnavigation (optional):* Die Linksnavigation bildet applikationsinterne Bereiche und Funktionsgruppen ab.
...
Die Enter löst die Primäraktion aus; Esc bricht die aktuelle Aktion ab.

[[r-detail-info]]
* *Informationsbereich (C, optional):*

** *Ein- und Ausblendung:* Der Informationsbereich ist initial ausgeblendet bereitzustellen und kann über einen eindeutig beschrifteten Button in der Seiten-Toolbar ein- und ausgeblendet werden.
...
** *Tabs und Accordion:* Tabs können eingesetzt werden, um gleichrangige Inhalte zu strukturieren und langes vertikales Scrollen zu vermeiden.
Accordions und Expander sind für nachgeordnete oder optionale Inhalte vorzusehen.
...
| <<r-dashboard-modalarm,Bestätigen/Löschen inline und Rückgängig-Option (Toast)>>
| WCAG 3.3.1 / 3.3.3 / 4.1.3
...
Optional kann ein oberer Hinweis- bzw. Validierungsbereich (Statusmeldung) verwendet werden.
...
* Drohender Datenverlust erfordert eine Bestätigung (z. B. „Ungespeicherte Änderungen verwerfen?“).

* Ein nicht fortsetzbarer Fehlerzustand ist eingetreten (z. B. Sitzung abgelaufen, Authentifizierung fehlgeschlagen und erneute Eingabe erforderlich).
...
* *Meldungs-Icon (B):* Das Meldungs-Icon ist optional und abhängig von der Severity; es unterstützt die Einordnung der Meldung (Info/Warnung/Fehler).
Farbe wird dabei nicht als einziges Unterscheidungsmerkmal verwendet.
...
Interaktionsmuster definieren wiederkehrende Abläufe und Rückmeldungen für häufige Use-Cases (z. B. Bestätigen & Rückgängig, Benachrichtigungen, Inline-Edit, Drill-Down, Assistent/Stepper).
Ziel ist eine effiziente und konsistente Bedienung mit möglichst wenigen modalen Unterbrechungen: Inline-, Drawer-, Popover- oder Drill-Down-Lösungen haben Vorrang vor blockierenden Dialogen.
Dazu gehören klare Aktionslabels, Rückgängig-Optionen wo möglich sowie feldnahe Validierung mit ergänzender Statusmeldung.

Die Muster sind vollständig per Tastatur und Screenreader bedienbar gemäß Kapitel <<barrierefreiheit>> und greifen auf Angular und PrimeNG/PrimeFlex zurück (Komponenten dienen als Referenz).
...
Bestätigungen und Rücknahmen erfolgen im Kontext, ohne den Arbeitsfluss oder die Lesereihenfolge unnötig zu unterbrechen.
...
Inline-Edit und Inline-Validation ist der Standard für fachliche Änderungen im aktuellen Sichtkontext und vorzugsweise einzusetzen.
...
Sobald diese Voraussetzungen nicht erfüllt sind, ist <<drilldown_detailansicht,Drill-Down>> zu verwenden.
...
In diesen Fällen ist <<drilldown_detailansicht,Drill-Down>> zu verwenden.

[[r-inlineedit-validierung]]
* *Validierung & Barrierefreiheit:* Die Validierung erfolgt feldnah (z. B. beim Verlassen des Feldes) oder beim Speichern; das Verhalten ist konsistent zu halten.
...
Die Tab-Reihenfolge folgt dem Lesefluss.
...
Der Drill-Down ist einzusetzen, wenn die Bearbeitung den Rahmen von Inline-Edit und Inline-Validation übersteigt.
...
* *Einsatzkriterien:* Drill-Down eignet sich bei vielen Feldern oder Gruppen, feldübergreifender Validierung, komplexen Abläufen (z. B. Rollen, Berechtigungen, mehrere Teilobjekte) sowie bei Anforderungen an Barrierefreiheit (klare Überschriften, Landmarks, stabile Navigation).

* *Navigation & History:* Die Browser-Navigation (Zurück/Vorwärts) funktioniert erwartungsgemäß ohne unerwartete Sprünge oder Datenverlust.
...
* *Barrierefreiheit & Fokusführung:* Beim Öffnen der Detailansicht erfolgt die Setzung eines sinnvollen Initialfokus (z. B. Seitenüberschrift oder erstes Eingabefeld).
Beim Zurückkehren liegt der Fokus auf dem auslösenden Element oder der bearbeiteten Zeile.

[[r-drilldown-validierung]]
* *Barrierefreiheit & Fehlerbehandlung:* Bei Fehlern erfolgt die Anzeige einer Zusammenfassung (z. B. „3 Felder prüfen“) mit Links zu den betroffenen Feldern.
...
* *Tastatur:* Die Tab-Reihenfolge folgt dem Lesefluss.
„Speichern“ ist per Tastatur erreichbar; Formular-Submit per Enter ist zulässig.
...
* *Responsives Verhalten:* Die Detailansicht bleibt bei 320 px Breite und 400 % Zoom ohne horizontales Scrollen nutzbar.
Abschnitte dürfen vertikal umbrechen (z. B. als Accordion oder Stack).
Aktionen bleiben jederzeit erreichbar.
...
* *Linearität & Navigation:* Standard ist ein linearer Stepper: „Weiter“ ist erst nach valider Eingabe aktiv.
...
* *Validierung & Feedback:* Die Validierung erfolgt feldnah (inline) mit verständlichen Fehlertexten, ergänzt um eine globale Meldungszone (`role` oder `aria-live`) pro Schritt.
...
* *Fokusführung & Tastatur:* Beim Schrittwechsel erfolgt die Setzung des Fokus auf die Schritt-Überschrift oder den ersten sinnvollen Inhalt.
Die Tab-Reihenfolge folgt der Sichtreihenfolge; Enter löst die Primäraktion aus, Esc bricht ab (sofern kein Datenverlust droht, andernfalls erfolgt eine Bestätigung im Kontext).
...
[[r-stepper-reflow]]
* *Zustände & Reflow:* Die Anzeige eines Skeleton erfolgt ab ca. 300 ms Ladezeit; Fehlerzustände enthalten einen hilfreichen Text und eine Option zum erneuten Versuch.
Layoutsprünge beim Wechsel der Zustände sind zu vermeiden; eine horizontale Scrollbar im Fließtext ist nicht zulässig.
Für die mobile Darstellung ist eine vertikale Anordnung vorgesehen; die Steuerleiste bleibt erreichbar und verschwindet nicht dauerhaft unterhalb des Viewports.

* *Zwischenspeichern & Abbruch:* Zwischenspeichern oder Ablegen ermöglicht die Unterbrechung und spätere Fortsetzung (z. B. Wiedervorlage im Dashboard).
...
* *Anti-Pattern (vermeiden):* Folgende Muster sind zu vermeiden:
Ein Stepper im modalen Dialog ist ohne zwingenden Grund nicht zulässig.
...
* *Tastatur:* Die Tab-Reihenfolge folgt dem Lesefluss.
Enter aktiviert die Primäraktion; Esc bricht ab oder löst eine Bestätigung aus (bei drohendem Datenverlust).
Nach erfolgreichem Schrittwechsel bleibt der Fokus stabil auf dem neuen Inhalt.

* *Screenreader:* Titel und Struktur benennen den aktuellen Schritt und den Gesamtfortschritt eindeutig.
Die Ankündigung von Statusmeldungen erfolgt über eine Live-Region ohne Fokusübernahme.

* *Responsives Verhalten:* Der Stepper bleibt bei 320 px Breite und 400 % Zoom ohne horizontales Scrollen nutzbar.
Die Schrittanzeige darf vertikal umbrechen; die Steuerleiste bleibt jederzeit erreichbar.
...
| <<r-bestaetigen-undo,Bestätigen & Rückgängig: Rückgängig-Option>>
| WCAG 3.3.4 / 4.1.3
