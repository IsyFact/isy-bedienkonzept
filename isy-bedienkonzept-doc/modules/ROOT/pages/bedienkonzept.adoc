...
* Die darin enthaltene Subnavigation (Menütimes der Hauptnavigation)
* Die Linksnavigation (Liste von Einträgen am linken Rand der Applikation)

Festlegungen auf höherer Ebene (zum Beispiel der Anwendungslandschaft) gelten für alle in ihr enthaltenen Fachanwendungen.
...
Je nach Komplexität der Anwendung oder Anwendungslandschaft gibt verschiedene Möglichkeiten eine Navigation zu gestalten.
...
Die Hauptnavigation bildet entsprechend der Subnavigation die Anwendung (oder logische Gruppe der Anwendungkomponenten) oder sogar eine Anwendungsgruppe ab.

Dies sieht in einfachen Fällen wie folgt aus:

* Anwendung (Hauptnavigation)

enthält

* Anwendungskomponente (Subnavigation)

enthält

* Use Case (Linksnavigation)

In komplexeren übergreifenden Szenarien kann eine Navigation wie folt aussehen:

* Anwendungsgruppen (Hauptnavigation)

enthält

* Anwendungen (Subnavigation)
...
Die Hauptnavigation wird mit Menüpunkten von links nach rechts befüllt.
...
Die Subnavigation wird mit Menüpunkten von oben nach unten befüllt.
Im optimalen Fall sind die Menüpunkte ihrer Priorität nach angeordnet.
...
Die Subnavigation wird zum Öffnen von Anwendungskomponenten oder Anwendungen genutzt.
Deren Navigation sich in der Linksnavigation fortsetzt.

[linksnavigation]
==== Linksnavigation

Die Linksnavigation ist eine vertikale Navigation, am linken Bildschirmrand.
Die Linksnavigation lässt sich ein und ausblenden.
...
[TIP]
Muss der Anwendende häufig bestimmte Funktionen oder Objekte zugreifen, so kann es sinnvoll sein im Schnellzugriffe einzurichten.
