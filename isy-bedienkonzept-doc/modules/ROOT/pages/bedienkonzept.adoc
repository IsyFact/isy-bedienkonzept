...
Weitere Panels innerhalb des Frontends werden durch ein aus-/einklappen nicht beeinflusst.
So können Nutzende alle Informationsgruppen eigenständig anzeigen oder bei Bedarf ausblenden lassen.
...
Technisch sind Panel-Widgets meist flexibler beziehungsweise nicht so restriktiv wie ein Akkordeon.
...
Für Nutzende ist eine solch differenzierte Betrachtung auf die technischen Elemente in den meisten Fällen nicht möglich und auch nicht notwendig.
Weshalb auf eine optische historisch gewachsene minimale Differenzierung der beiden Elemente zu Gunsten der Usability verzichtet werden kann.
...
Zur Vereinfachung der Usability wird empfohlen die <<design-pattern>> Akkordeon und Panel optisch anzunähern.
Beide Elemente sollten sich über das gleiche Symbol aus- und einklappen lassen.
Im IsyFact Kontext ist dies ein Chevron beziehungsweise Pfeil-rechts und Pfeil-unten.
Die Funktion sollte links vom Label gegeben sein.

Weiterführende Aktionen des Panels sollten sich rechtsbündig befinden oder in einer Footer Sektion nach dem Inhaltsbereich.
Beim Akkordeon soll auf weiterführende Interaktion, über das aus- und einklappen hinaus, verzichtet werden.
