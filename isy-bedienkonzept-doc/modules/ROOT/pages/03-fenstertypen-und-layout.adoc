...
Fenstertypen und Layout bilden die verbindliche Grundlage für die Struktur und das Verhalten aller Seiten.
Ziel ist eine einheitliche und effiziente Nutzererfahrung in modernen Fachanwendungen.

Die in Kapitel 2 xref:02-barrierefreiheit.adoc#barrierefreiheit[Barrierefreiheit] beschriebenen Anforderungen der WCAG 2.1 AA gelten uneingeschränkt und bilden den verbindlichen Rahmen.

Die Vorgaben dieses Kapitels sind technologieunabhängig formuliert und beschreiben das erwartete Verhalten sowie die gestalterischen Anforderungen an Layout und Responsivität.
Grundlegende Interaktionsmuster werden im Kapitel xref:07-design-pattern.adoc#design-pattern[Design Patterns] beschrieben.

Die folgenden Gestaltungsprinzipien definieren zentrale Regeln für Struktur, Lesbarkeit und Anpassungsfähigkeit der Benutzeroberfläche und sichern ein konsistentes Erscheinungsbild über alle Fenstertypen hinweg.
...
Das Layout und der Inhalt müssen auch bei Größenänderungen nutzbar bleiben.
Reflows dürfen die Orientierung der Nutzenden nicht beeinträchtigen.
Das Verhalten folgt den allgemeinen <<gestaltungsprinzipien, Gestaltungsprinzipien>>.
...
* *Tastaturbedienung:* Die Fokusreihenfolge entspricht der visuellen Lesereihenfolge und bildet eine durchgängige „Focus Order“.
...
* *Stabilität:* Leere, Lade- oder Fehlerzustände belegen konstanten Raum, damit das Layout ruhig und stabil bleibt.
...
* *Zielgrößen:* Interaktive Elemente müssen mindestens 24 x 24 CSS-Pixel groß sein.
Für eine komfortable Touch-Bedienung wird eine Größe von 44 x 44 CSS-Pixel empfohlen.
...
Die spezifischen Fenster- und Seitentypen (z. B. Login, Dashboard, Dashboard-Unterseite, Applikationsseite, Applikations-Detailseite) sind im Kapitel Design Patterns (xref:07-design-pattern.adoc#primaere-fenstertypen[Primäre Fenstertypen] und xref:07-design-pattern.adoc#sekundaere-fenstertypen[Sekundäre Fenstertypen]) beschrieben.
...
* *E/F* Haupt- und Subnavigation als Flyout (zur Nutzung siehe xref:07-design-pattern.adoc#hauptnavigation[Hauptnavigation])
...
Er nutzt ein gemeinsames, responsives Gestaltungsraster, das eine konsistente und verständliche Anordnung von Komponenten, Formularen und Navigationselementen über alle Fenstertypen hinweg gewährleistet.

Im Inhaltsbereich werden sowohl Fachinhalte als auch interaktive Elemente angezeigt.
Die Gestaltung orientiert sich an den Prinzipien aus Kapitel <<layout-resizing>> (z. B. Reflow und Spacing) und berücksichtigt insbesondere:
...
Die konkreten Fenster- und Seitentypen, die dieses gemeinsame Raster verwenden, sind in Kapitel xref:07-design-pattern.adoc#primaere-fenstertypen[Primäre Fenstertypen] detailliert beschrieben:

** xref:07-design-pattern.adoc#login[Login]
** xref:07-design-pattern.adoc#portalstartseite[Portalstartseite]
** xref:07-design-pattern.adoc#dashboard-unterseite[Dashboard-Unterseite]
** xref:07-design-pattern.adoc#applikationsseite[Applikationsseite]
** xref:07-design-pattern.adoc#applikations-detailseite[Applikations-Detailseite]

[[nicht-modale-und-kontextbezogene-interaktionen]]
=== Nicht-modale und kontextbezogene Interaktionen (Grundsatz)
...
Da modale Dialoge den Arbeitsfluss unterbrechen und die Orientierung erschweren können, sollen sie sparsam eingesetzt werden.
Interaktionen sollen bevorzugt nicht-modal und im Kontext der aktuellen Aufgabe stattfinden, sofern dies fachlich möglich ist.

Dieses Prinzip verbessert Effizienz, Barrierefreiheit und die allgemeine Nutzererfahrung.

*_Grundprinzip_*

* *Bevorzugte Muster:* Statt vollständig modaler Fenster werden bevorzugt kontextbezogene und möglichst nicht-blockierende Interaktionsmuster eingesetzt - beispielsweise Inline-Bestätigungsleisten, nicht-modale Drawer, Popover, Tooltips oder aufklappbare Bereiche (Accordions/Expander).

* *Modale Dialoge nur bei zwingendem Bedarf:* Ein modaler Dialog darf nur verwendet werden, wenn eine Unterbrechung des Arbeitsflusses fachlich, rechtlich oder sicherheitsrelevant erforderlich ist - etwa bei kritischen, destruktiven oder bestätigungspflichtigen Aktionen.
Solche Fälle müssen im Design-Review begründet werden.

*_Fokus-Management_*

Die folgenden Regeln gelten für modale Dialoge sowie für vergleichbare interaktive Bereiche wie Drawer, Popover oder aufklappbare Inhaltsbereiche.
...
Die folgenden Anforderungen unterstützen das Prinzip nicht-modaler und kontextbezogener Interaktion, gelten aber grundsätzlich auch für andere interaktive Bereiche im Frontend.
