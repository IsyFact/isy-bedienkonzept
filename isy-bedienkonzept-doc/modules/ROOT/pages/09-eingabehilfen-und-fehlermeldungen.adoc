...
Eingabehilfen und Fehlermeldungen sind essenziell wichtig, um die xref:01-einfuehrung.adoc#usability-und-benutzerfreundlichkeit[Benutzerfreundlichkeit] eines Frontends zu erhöhen.
...
Auch xref:04-basiselemente.adoc#label[Labels] gelten als Eingabehilfen.
Klare und präzise Beschriftungen (Labels) sind für die Benutzerführung entscheidend, insbesondere für xref:05-benutzereingaben.adoc#benutzereingaben[Benutzereingaben] und xref:06-bedienelemente.adoc#eingabefeld[Eingabefelder].
...
Sie sollten jedoch nicht als Ersatz für xref:04-basiselemente.adoc#label[Labels] dienen.
Idealerweise sollten Platzhaltertexte kurz und prägnant sein, um den Nutzenden zu helfen, die Anforderungen des jeweiligen xref:06-bedienelemente.adoc#eingabefeld[Eingabefelds] besser zu verstehen, ohne den Platz zu überladen oder den Fokus zu stören.
...
Sobald die Nutzenden in das Eingabefeld klicken, wird der Platzhalter ausgeblendet und es besteht die Möglichkeit, eine Mask anzeigen zu lassen (siehe Kapitel xref:06-bedienelemente.adoc#eingabefeld[Eingabefelder]).
...
Bei komplexeren xref:05-benutzereingaben.adoc#benutzereingaben[Formularen] oder spezifischen Eingabeanforderungen können Hinweistexte hilfreich sein, um den Nutzenden zusätzliche Anleitungen oder Kontext zu bieten.
...
Autosuggestion ist eine Zusatzfunktion, die in xref:06-bedienelemente.adoc#eingabefeld[Eingabefeldern] ergänzt werden kann.
...
Neben Fehlermeldungen, die am Eingabeobjekt auftreten, kann zusätzlich ein zusammenfassender Hinweis angezeigt werden.
...
Um die xref:02-barrierefreiheit.adoc#barrierefreiheit[Barrierefreiheit] einzuhalten, muss die Benachrichtigung dauerhaft eingeblendet bleiben, bis sie von den Nutzenden geschlossen wird, siehe https://www.w3.org/WAI/WCAG21/Understanding/content-on-hover-or-focus.html[WCAG 2.1 SC 1.4.13].
...
Siehe xref:07-design-pattern.adoc#meldungsdialoge[Meldungsdialoge], xref:07-design-pattern.adoc#benachrichtigungen[Benachrichtigungen] und xref:07-design-pattern.adoc#container-overlays[Container und Overlays].
...
Idealerweise sollten Fehlermeldungen in unmittelbarer Nähe von betroffenen Objekten platziert werden (siehe als Beispiel: Eingabefelder - Fehlermeldung), um Nutzenden sofortiges Feedback zu geben und sie bei der Fehlerbehebung zu unterstützen.
